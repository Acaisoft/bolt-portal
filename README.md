...
REACT_APP_AUTH_SERVICE_URL=<url to auth service login page>
```

`REACT_APP_AUTH_SERVICE` currently supports two options: `bolt` and `keycloak`. If no value will be specified, application by default will use bolt auth service. If value is set to `keycloak`, `REACT_APP_AUTH_SERVICE_URL` will not have any effect on the app because Keycloak handles login and redirecting to its login page by itself.
